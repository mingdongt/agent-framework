...
from af_fix.claude_agent_runner import (
    ClaudeAgentRunner,
...
def test_runner_returns_gave_up_when_marker_present(tmp_path: Path) -> None:
    ...
    def fake_run_claude(**kwargs: object) -> object:
        return fake_state

    runner = ClaudeAgentRunner(
        model="claude-opus-4-7",
        max_turns=80,
        _run_claude=fake_run_claude,
        _git_diff=lambda ws: "",
    )
...
def test_runner_returns_success_when_diff_nonempty(tmp_path: Path) -> None:
    ...
    runner = ClaudeAgentRunner(
        model="claude-opus-4-7",
        max_turns=80,
        _run_claude=lambda **kw: fake_state,
...
def test_runner_returns_failure_when_run_claude_raises(tmp_path: Path) -> None:
    workspace = tmp_path / "ws"
    workspace.mkdir()

    def bad(**kw: object) -> object:
        raise RuntimeError("claude agent error: connection failed")
    ...
    result = runner.run(issue=_issue(), workspace=workspace)
    assert result.success is False
    assert "connection failed" in (result.reason or "")
